...
if(isset($_POST['submit'])){
    ...
    $params = array();
    $img_path = $image->upload_image('image', 'accessoryImages');
    $params['name'] =  $security->post('name');
    $params['category'] =  $security->post('category_id');
    ...
    if($img_path == 'type_error'){
        ...
        if($accessory->store($params)){
            $info = $notification->success('Accessory added successfully');
        }else{
...
$template->filename = 'views/backoffice/addAccessory.tpl';
...
$template->add_snippet('page_title', 'Add Accessory');
