...
class AccessoryCat
{
    ...
    public function categories_links(){
       ...
       if($run_query){
           ...
           if($this->db->num_rows($run_query) > 0){
               do{
                   $links.='<li><a href="'.BASE_URL.'accessories/'.$fetch_query['id'].'/1'.'">'.$fetch_query['name'].'</a></li>';
               }while($fetch_query = $this->db->fetch($run_query));
           }
       }
       return $links;
    ...
    public function storeCategory($params , $print=false){
        $query = "INSERT INTO accessories_cat (`name`, `price`) VALUES ('{$params['name']}', '{$params['price']}')";
        if($print){
            echo $query;
        }
    ...
    public function categoryName($category_id){
        return $this->db->fetch_by_field('name', 'accessories_cat', " id = {$category_id}");
    }
